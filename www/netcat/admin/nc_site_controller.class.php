...
class nc_site_controller extends nc_ui_controller {
    ...
    protected function get_site_request_url($action) {
        ...
        $store_request_url = "http://store.netcat.ru/api/v1/site/$action/?" . $store_request_parameters;
    ...
    protected function action_create() {
        ...
        if ($site_type == 'blank') {
            ...
            $site_properties = $nc_core->catalogue->get_by_id($created_site_id, null, true);
    ...
    protected function action_download_and_import() {
        ...
        // Загрузка информации о новом сайте / сброс кэша
        $nc_core->catalogue->get_by_id($created_site_id, null, true);
                    ...
                    array(
                        ...
                        'buttons' => array(
                            array(
                                ...
                                "action" => "window.open('http://".(($site_properties['Domain']) ? $site_properties['Domain'] : $_SERVER['HTTP_HOST'])."');",
                                "icon" => "arrow-right",
                                "sprite" => true,
                            ),
                            array(
